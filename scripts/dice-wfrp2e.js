...
class DiceWFRP
{
  ...
  })
  {
    ...
    if (game.settings.get("wfrp2e", "testDefaultDifficulty") && (game.combat != null))
      sceneStress = game.combat.started ? "challenging" : "average";
    else if (game.settings.get("wfrp2e", "testDefaultDifficulty"))
      sceneStress = "average";
    ...
    let advantageBonus = game.settings.get("wfrp2e", "autoFillAdvantage") ? (dialogOptions.data.advantage * 10 || 0) : 0

    mergeObject(dialogOptions.data,
    {
      testDifficulty: dialogOptions.data.testDifficulty || sceneStress,
      difficultyLabels: WFRP2E.difficultyLabels,
  ...
  static rollTest(testData)
  {
    ...
    else if (roll.total <= 5 || roll.total <= targetNum)
    {
      ...
      if (game.settings.get("wfrp2e", "fastSL"))
      {
      ...
      if (game.settings.get("wfrp2e", "testAbove100"))
      {
        if (targetNum > 100)
        {
    ...
    if (game.settings.get("wfrp2e", "criticalsFumblesOnAllTests") && !testData.hitLocation)
    {
      if (roll.total > targetNum && roll.total % 11 == 0 || roll.total == 100)
      {
        rollResults.extra.color_red = true;
  ...
  static rollCastTest(testData)
  {
    ...
    if (game.settings.get("wfrp2e", "partialChannelling"))
    {
  ...
  static rollChannellTest(testData, actor)
  {
    ...
    if (testResults.description.includes(game.i18n.localize("Failure")))
    {
      ...
      if (Number(SL) == 0 && game.settings.get("wfrp2e", "extendedTests"))
        SL = -1;
  ...
  static async renderRollCard(chatOptions, testData, rerenderMessage)
  {
    ...
    if (game.settings.get("wfrp2e", "manualChatCards") && !rerenderMessage)
    ...
    if (!rerenderMessage)
    {
      ...
      return renderTemplate(chatOptions.template, chatData).then(html =>
      {
        ...
        if (game.settings.get("wfrp2e", "manualChatCards"))
        {
          let blank = $(html)
        ...
        if (chatOptions.sound)
          console.log(`wfrp2e | Playing Sound: ${chatOptions.sound}`)
  ...
  static async chatListeners(html)
  {
    ...
    html.on('change', '.card-edit', ev =>
    {
      ...
      ActorWfrp2e[`${data.postData.postFunction}`](newTestData, chatOptions, message);
    })
    ...
    html.on("click", '.chargen-button, .chargen-button-nostyle', event =>
    {
      ...
      {
        case "rollSpecies":
          GeneratorWfrp2e.rollSpecies($(event.currentTarget).parents('.message').attr("data-message-id"))
          break;
        case "rollCareer":
          GeneratorWfrp2e.rollCareer($(event.currentTarget).attr("data-species"), WFRP2E.randomExp.careerRand)
          break;
        case "rerollCareer":
          GeneratorWfrp2e.rollCareer($(event.currentTarget).attr("data-species"), WFRP2E.randomExp.careerReroll, true)
          GeneratorWfrp2e.rollCareer($(event.currentTarget).attr("data-species"), WFRP2E.randomExp.careerReroll, true)
          break;
        case "chooseCareer":
          GeneratorWfrp2e.chooseCareer($(event.currentTarget).attr("data-species"))
          break;
        case "rollSpeciesSkillsTalents":
          GeneratorWfrp2e.speciesSkillsTalents($(event.currentTarget).attr("data-species"))
          break;
        case "rollDetails":
          GeneratorWfrp2e.rollDetails($(event.currentTarget).attr("data-species"))
          break;

        case "rerollAttributes":
          GeneratorWfrp2e.rollAttributes($(event.currentTarget).attr("data-species"), Number($(event.currentTarget).attr("data-exp")), true)
          break;
      }
    });
    ...
    html.on("click", '.career-select', event =>
    {
      ...
      GeneratorWfrp2e.displayCareer(careerSelected, species, 0, false, true)
    });
      ...
      switch ($(event.currentTarget).attr("data-button"))
      {
        ...
        case "payItem":
          if(!game.user.isGM)
          {
            ...
            money = MarketWfrp2e.payCommand($(event.currentTarget).attr("data-pay"), money);
            if(money)
            {
        ...
        case "creditItem":
          if(!game.user.isGM)
          {
            ...
            money = MarketWfrp2e.creditCommand(dataExchange, money);
            if(money)
            {
