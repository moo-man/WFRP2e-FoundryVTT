 ...
 *
 * ActorWfrp2e contains all the preparation data and methods used for preparing an actor:
 ...
 *
 *
 * @see   ActorSheetWfrp2e - Base sheet class
 * @see   ActorSheetWfrp2eCharacter - Character sheet class
 * @see   ActorSheetWfrp2eNPC - NPC sheet class
 * @see   ActorSheetWfrp2eCreature - Creature sheet class
 * @see   DiceWFRP2e - Sends test data to roll tests.
 */
class ActorWfrp2e extends Actor {

  /**
   * Override the create() function to provide additional WFRP2e functionality.
   *
  ...
  setupCharacteristic(characteristicId, options = {}) {
    ...
    let dialogOptions = {
      title: title,
      template : "/systems/wfrp2e/templates/chat/characteristic-dialog.html",
      ...
      callback : (html, roll) => {
        ...
        testData.testDifficulty = WFRP2E.difficultyModifiers[html.find('[name="testDifficulty"]').val()];
    ...
    let cardOptions = this._setupCardOptions("systems/wfrp2e/templates/chat/characteristic-card.html", title)
  ...
  setupSkill(skill, options = {}) {
    ...
    let dialogOptions = {
      title: title,
      template : "/systems/wfrp2e/templates/chat/skill-dialog.html",
      // Prefilled dialog data

      data : {
        hitLocation : testData.hitLocation,
        talents : this.data.flags.talentTests,
        characteristicList : WFRP2E.characteristics,
    ...
    // Call the universal cardOptions helper
    let cardOptions = this._setupCardOptions("systems/wfrp2e/templates/chat/skill-card.html", title)
  ...
  setupWeapon(weapon, options = {}) {
    ...
    if (game.settings.get("wfrp2e", "testAutoFill") && (game.combat && game.combat.data.round != 0 && game.combat.turns))
    {
      try
      {
    ...
    let dialogOptions = {
      title: title,
      template : "/systems/wfrp2e/templates/chat/weapon-dialog.html",
      // Prefilled dialog data
      data : {
        hitLocation : testData.hitLocation,
        talents : this.data.flags.talentTests,
    ...
    // Call the universal cardOptions helper
    let cardOptions = this._setupCardOptions("systems/wfrp2e/templates/chat/weapon-card.html", title)
  ...
  spellDialog(spell, options={}) {
    ...
    {
      renderTemplate("systems/wfrp2e/templates/chat/cast-channel-dialog.html").then(dlg => {
        new Dialog({
  ...
  setupCast(spell, options = {}) {
    ...
    let dialogOptions = {
      title: title,
      template : "/systems/wfrp2e/templates/chat/spell-dialog.html",
      // Prefilled dialog data
      data : {
        hitLocation : testData.hitLocation,
    ...
    // Call the universal cardOptions helper
    let cardOptions = this._setupCardOptions("systems/wfrp2e/templates/chat/spell-card.html", title)
  ...
  setupChannell(spell, options = {}){
    ...
    {
      defaultSelection = channellSkills.indexOf(channellSkills.find(x => x.name.includes(WFRP2E.magicWind[spellLore])));
    }
    ...
    let dialogOptions = {
      title: title,
      template : "/systems/wfrp2e/templates/chat/channel-dialog.html",
      // Prefilled dialog data
      data : {
        malignantInfluence : testData.malignantInfluence,
    ...
    // Call the universal cardOptions helper
    let cardOptions = this._setupCardOptions("systems/wfrp2e/templates/chat/channel-card.html", title)
  ...
  setupPrayer(prayer, options = {}) {
    ...
    let dialogOptions = {
      title: title,
      template : "/systems/wfrp2e/templates/chat/prayer-dialog.html",
      // Prefilled dialog data
      data : {
        hitLocation : testData.hitLocation,
        talents : this.data.flags.talentTests,
    ...
    // Call the universal cardOptions helper
    let cardOptions = this._setupCardOptions("systems/wfrp2e/templates/chat/prayer-card.html", title)
  ...
  setupTrait(trait, options={}) {
    if (!trait.data.rollable.value)
      return;
    let title =   WFRP2E.characteristics[trait.data.rollable.rollCharacteristic] + ` ${game.i18n.localize("Test")} - ` + trait.name;
    let testData = {
      hitLocation : false,
    ...
    let dialogOptions = {
      title: title,
      template : "/systems/wfrp2e/templates/chat/skill-dialog.html", // Reuse skill dialog
      // Prefilled dialog data
      data : {
        hitLocation : testData.hitLocation,
        talents : this.data.flags.talentTests,
  ...
  static async  defaultRoll(testData, cardOptions, rerenderMessage = null) {
   ...
   { }
    Hooks.call("wfrp2e:rollTest", result, cardOptions)
  ...
  static async incomeOverride(testData, cardOptions, rerenderMessage = null)
  {
    ...
    Hooks.call("wfrp2e:rollIncomeTest", result, cardOptions)
    ...
    let dieAmount = WFRP2E.earningValues[WFRP_Utility.findKey(status[0], WFRP2E.statusTiers)][0] // b, s, or g maps to 2d10, 1d10, or 1 respectively (takes the first letter)
    dieAmount = Number(dieAmount) * status[1];     // Multilpy that first letter by your standing (Brass 4 = 8d10 pennies)
    let moneyEarned;
    if (WFRP_Utility.findKey(status[0], WFRP2E.statusTiers) != "g") // Don't roll for gold, just use standing value
    {
    ...
    if (result.description.includes("Success"))
    {
      ...
      switch (WFRP_Utility.findKey(status[0], WFRP2E.statusTiers))
      { 
        case "b":
    ...
    // let contextAudio = await WFRP_Audio.MatchContextAudio(WFRP_Audio.FindContext(result))
    // cardOptions.sound = contextAudio.file || cardOptions.sound
    result.moneyEarned = moneyEarned + WFRP_Utility.findKey(status[0], WFRP2E.statusTiers);
    DiceWFRP.renderRollCard(cardOptions, result, rerenderMessage).then(msg => {
  ...
  static async weaponOverride(testData, cardOptions, rerenderMessage = null)
  {
   ...
   { }
    Hooks.call("wfrp2e:rollWeaponTest", result, cardOptions)
  ...
  static async castOverride(testData, cardOptions, rerenderMessage = null)
  {
   ...
   { }
    Hooks.call("wfrp2e:rollCastTest", result, cardOptions)
  ...
  static async channellOverride(testData, cardOptions, rerenderMessage = null)
  {
   ...
   { }
    Hooks.call("wfrp2e:rollChannelTest", result, cardOptions)
  ...
  static async prayerOverride(testData, cardOptions, rerenderMessage = null)
  {
   ...
   { }
    Hooks.call("wfrp2e:rollPrayerTest", result, cardOptions)
  ...
  static async traitOverride(testData, cardOptions, rerenderMessage = null)
  {
   ...
   { }
    Hooks.call("wfrp2e:rollTraitTest", result, cardOptions)
   ...
   * for display (getData() - see ActorSheetWfrp2e) and other needs. This is where all items (call to 
    ...
    for (let career of actorData.careers)
    {
      if (career.data.current.value)
      {
        ...
        if (!actorData.data.details.status.value) // backwards compatible with moving this to the career change handler
          actorData.data.details.status.value = WFRP2E.statusTiers[career.data.status.tier] + " " + career.data.status.standing;
  ...
  prepareItems()
  {
    ...
    const inventory = {
      weapons: {
        label: game.i18n.localize("WFRP2E.TrappingType.Weapon"), // Label - what is displayed in the inventory section header
      ...
      armor: {
        label: game.i18n.localize("WFRP2E.TrappingType.Armour"),
        items: [],
        toggle: true,
      ...
      ammunition: {
        label: game.i18n.localize("WFRP2E.TrappingType.Ammunition"),
        items: [],
        show: false,            
        dataType: "ammunition"
      },
      clothingAccessories: {
        label: game.i18n.localize("WFRP2E.TrappingType.ClothingAccessories"),
        items: [],
        toggle: true,
        toggleName: game.i18n.localize("Worn"),
        show: false,
      ...
      booksAndDocuments: {
        label: game.i18n.localize("WFRP2E.TrappingType.BooksDocuments"),
        items: [],
        show: false,
        dataType: "trapping"
      },
      toolsAndKits: {
        label: game.i18n.localize("WFRP2E.TrappingType.ToolsKits"),
        items: [],
        show: false,
        dataType: "trapping"
      },
      foodAndDrink: {
        label: game.i18n.localize("WFRP2E.TrappingType.TradeTools"),
        items: [],
        show: false,
        dataType: "trapping"
      },
      drugsPoisonsHerbsDraughts: {
        label: game.i18n.localize("WFRP2E.TrappingType.DrugsPoisonsHerbsDraughts"),
        items: [],
        show: false,
        dataType: "trapping"
      },
      misc: {
        label: game.i18n.localize("WFRP2E.TrappingType.Misc"),
        items: [],
        show: true,
        dataType: "trapping"
      }
    ...
    const ingredients = {
      label: game.i18n.localize("WFRP2E.TrappingType.Ingredient"),
      items: [],
      show: false,
      dataType: "trapping"
    };
    ...
    for (var cont of containers.items) // For each container
    {
      ...
      { // Add category of item to be displayed
        if (item.type == "trapping")
          item.type = WFRP2E.trappingCategories[item.data.trappingType.value];
        else
          item.type = WFRP2E.trappingCategories[item.type];
      })
    ...
    if (enc.state > 3) 
    {
      ...
      enc.penalty = WFRP2E.encumbrancePenalties["maxEncumbered"];
    } 
    else if (enc.state > 2) 
    {
      enc["veryEncumbered"] = true
      enc.penalty = WFRP2E.encumbrancePenalties["veryEncumbered"];
    } 
    else if (enc.state > 1) 
    {
      enc["encumbered"] = true
      enc.penalty = WFRP2E.encumbrancePenalties["encumbered"];
    } 
    else
  ...
  prepareSkill(skill) 
  {
    ...
    skill.data.characteristic.abrev = WFRP2E.characteristicsAbbrev[skill.data.characteristic.value];
  ...
  prepareWeaponCombat(weapon, ammoList, skills)
  {
    ...
    weapon.attackType = WFRP2E.groupToType[weapon.data.weaponGroup.value]
    weapon.data.reach.value = WFRP2E.weaponReaches[weapon.data.reach.value];
    weapon.data.weaponGroup.value = WFRP2E.weaponGroups[weapon.data.weaponGroup.value] || "basic";
  ...
  calculateSpellAttributes(formula, aoe=false)
  {
    ...
    if (formula != game.i18n.localize("You").toLowerCase() && formula != game.i18n.localize("Special").toLowerCase() && formula != game.i18n.localize("Instant").toLowerCase())
    {
      ...
      for(let ch in actorData.data.characteristics)
      {
        ...
        if (formula.includes(WFRP2E.characteristics[ch].toLowerCase()))
        {
          // Determine if it's looking for the bonus or the value
          if (formula.includes('bonus'))
            formula = formula.replace(WFRP2E.characteristics[ch].toLowerCase().concat(" bonus"),  actorData.data.characteristics[ch].bonus);
          else
            formula = formula.replace(WFRP2E.characteristics[ch].toLowerCase(),  actorData.data.characteristics[ch].value);
        }
      }
    }
  ...
  static applyDamage(victim, opposeData, damageType = DAMAGE_TYPE.NORMAL)
  {
    ...
    if (applyAP)
    {
      ...
      catch (e) {console.log("wfrp2e | Sound Context Error: " + e)} // Ignore sound errors
    }
    else updateMsg += ")"
    ...
    if (newWounds <= 0 && !impenetrable)
    {
      ...
      let critAmnt = game.settings.get("wfrp2e", "dangerousCritsMod")
      if(game.settings.get("wfrp2e", "dangerousCrits") && critAmnt && (Math.abs(newWounds) - actor.data.data.characteristics.t.bonus) > 0)
      {
    ...
    if (hack)
      updateMsg += `<br>${game.i18n.localize("CHAT.DamageAP")} ${WFRP2E.locations[opposeData.hitloc.value]}`
  ...
  async _advanceSpeciesSkills() 
  {
    ...
    try
    {
      skillList = WFRP2E.speciesSkills[this.data.data.details.species.value];
      if (!skillList)
      {
        ...
        skillList = WFRP2E.speciesSkills[WFRP_Utility.findKey(this.data.data.details.species.value, WFRP2E.species)]
        if (!skillList)
        {
    ...
    catch(error)
    {
        ...
        console.log("wfrp2e | Could not find species " + this.data.data.details.species.value + ": " + error);
        throw error
    }
  ...
  async _advanceSpeciesTalents() 
  {
    ...
    try
    {
      talentList = WFRP2E.speciesTalents[this.data.data.details.species.value];
      if (!talentList)
      {
        // findKey() will do an inverse lookup of the species key in the species object defined in config.js, and use that if 
        // user-entered species value does not work (which it probably will not)
        talentList = WFRP2E.speciesTalents[WFRP_Utility.findKey(this.data.data.details.species.value, WFRP2E.species)]
        if (!talentList)
  ...
  useFortuneOnRoll(message, type)
  {
    if(this.data.data.status.fortune.value > 0)
    {
      ...
      if(type=="reroll")
      {
        ...
        ActorWfrp2e[data.postData.postFunction](data.preData,cardOptions);
      ...
      else //addSL
      {
        ...
        ActorWfrp2e[data.postData.postFunction](newTestData,cardOptions,message);
        message.update({
          "flags.data.fortuneUsedAddSL" : true
        });
      }
  ...
  useDarkDeal(message)
  {
    ...
    ActorWfrp2e[message.data.flags.data.postData.postFunction](message.data.flags.data.preData,cardOptions);
  }
...
CONFIG.Actor.entityClass = ActorWfrp2e;
...
Hooks.on("preUpdateActor", (data, updatedData) =>{
  if (data.data.token.img == "systems/wfrp2e/tokens/unknown.png" && updatedData.img)
  {
