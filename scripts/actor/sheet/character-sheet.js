 ...
 *
 * ActorSheetWfrp2eCharacter are assigned to character type actors, and the specific interactions
 * character type actors need are defined here, specifically for careers and spending exp.
 * 
 */
class ActorSheetWfrp2eCharacter extends ActorSheetWfrp2e
{
  static get defaultOptions()
  {
    ...
    mergeObject(options,
    {
      classes: options.classes.concat(["wfrp2e", "actor", "character-sheet"]),
      width: 610,
      height: 740,
    });
    return options;
  ...
  get template()
  {
    if (!game.user.isGM && this.actor.limited) return "systems/wfrp2e/templates/actors/actor-limited.html";
    return "systems/wfrp2e/templates/actors/actor-sheet.html";
    ...
    html.find('.career-toggle').click(async ev =>
    {
      ...
      if (type == "current")
      {
        ...
        if (item.data.current.value)
        {
          ...
          this.actor.update({"data.details.status.value" : WFRP2E.statusTiers[item.data.status.tier] + " " + item.data.status.standing})
        }
        else
        {
          for (let char in characteristics)
...
Actors.registerSheet("wfrp2e", ActorSheetWfrp2eCharacter,
{
  types: ["character"],
  makeDefault: true
});