 ...
 *
 * ActorSheetWfrp2e provides the general interaction and data organization shared among all 
 ...
 *
 * @see   ActorWfrp2e - Data and main computation model (this.actor)
 ...
 */
class ActorSheetWfrp2e extends ActorSheet {
   ...
   * to organize, process, and prepare all actor data for display. See ActorWfrp2e.prepare()
   * 
  ...
  html.find('.item-delete').click(ev => {
    let li = $(ev.currentTarget).parents(".item"),
      ...
      {
        AudioHelper.play({src : "systems/wfrp2e/sounds/squeek.wav"}, false)
        return // :^)
      }
      
      renderTemplate('systems/wfrp2e/templates/chat/delete-item-dialog.html').then(html => {
        new Dialog({
  ...
  ******************************************************/
  ...
  html.find('.input.species').change(async event => {
    if (this.actor.data.type == "character")
      return
    if (game.settings.get("wfrp2e", "npcSpeciesCharacteristics"))
    {
  ...
  html.find('.randomize').click(async event => {
    ...
    {
      console.log("wfrp2e | Could not randomize: " + error)
    }
  ...
  html.find('.dollar-icon').click(async event => {
    ...
    money = MarketWfrp2e.consolidateMoney(money);
  ...
  { 
    ...
    else if (JSON.parse(dragData).generation)
    {
      ...
      if (transfer.type == "attributes") // Characteristsics, movement, metacurrency, etc.
      {
        ...
        for (let c in WFRP2E.characteristics)
        {
  ...
  _expandProperty(event)
  {
    ...
    let li = $(event.currentTarget).parents(".item"),
      ...
      propertyDescr = Object.assign(duplicate(WFRP2E.qualityDescriptions), WFRP2E.flawDescriptions); // Property descriptions
  ...
  _expandInfo(event)
  {
    ...
    if (classes.hasClass("weapon-range"))
    {
      ...
      expansionText =
        `<a class="range-click" data-range="easy">0 yd - ${Math.ceil(range / 10)} ${game.i18n.localize("yds")}: ${WFRP2E.rangeModifiers["Point Blank"]}</a><br>
          <a class="range-click" data-range="average">${(Math.ceil(range / 10) + 1)} ${game.i18n.localize("yds")} - ${Math.ceil(range / 2)} ${game.i18n.localize("yds")}: ${WFRP2E.rangeModifiers["Short Range"]}</a><br>
          <a class="range-click" data-range="challenging">${(Math.ceil(range / 2) + 1)} ${game.i18n.localize("yds")} - ${range} yds: ${WFRP2E.rangeModifiers["Normal"]}</a><br>
          <a class="range-click" data-range="difficult">${(range + 1)} ${game.i18n.localize("yds")} - ${range * 2} ${game.i18n.localize("yds")}: ${WFRP2E.rangeModifiers["Long Range"]}</a><br>
          <a class="range-click" data-range="vhard">${(range * 2 + 1)} ${game.i18n.localize("yds")} - ${range * 3} ${game.i18n.localize("yds")}: ${WFRP2E.rangeModifiers["Extreme"]}</a><br>`;
    }
    ...
    else if (classes.hasClass("weapon-group"))
    {
      ...
      let weaponGroupKey = "";
      weaponGroupKey = WFRP_Utility.findKey(weaponGroup, WFRP2E.weaponGroups);
      expansionText = WFRP2E.weaponGroupDescriptions[weaponGroupKey];
    }
    ...
    else if (classes.hasClass("weapon-reach"))
    {
      ...
      let reachKey;
      reachKey = WFRP_Utility.findKey(reach, WFRP2E.weaponReaches);
      expansionText = WFRP2E.reachDescription[reachKey];
    }
