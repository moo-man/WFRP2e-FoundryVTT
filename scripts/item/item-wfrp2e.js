/**
 * The ItemWfrp2e class only provides two core functionalities
 * 
...
class ItemWfrp2e extends Item
{
  ...
  _mutationExpandData()
  {
    const data = duplicate(this.data.data);
    data.properties = [];
    data.properties.push(WFRP2E.mutationTypes[this.data.data.mutationType.value]);
  ...
  _careerExpandData()
  {
    ...
    data.properties.push(WFRP2E.statusTiers[this.data.data.status.tier] + " " + this.data.data.status.standing);
    data.properties.push(`<b>${game.i18n.localize("Characteristics")}</b>: ${this.data.data.characteristics.map(i => i = " " + WFRP2E.characteristicsAbbrev[i])}`);
  ...
  _weaponExpandData()
  {
    ...
    if (data.weaponGroup.value)
      properties.push(WFRP2E.weaponGroups[data.weaponGroup.value]);
    if (data.range.value)
    ...
    if (data.reach.value)
      properties.push(`${game.i18n.localize("Reach")}: ${WFRP2E.weaponReaches[data.reach.value] + " - " + WFRP2E.reachDescription[data.reach.value]}`);
  ...
  _armourExpandData()
  {
    const data = duplicate(this.data.data);
    const properties = [];
    properties.push(WFRP2E.armorTypes[data.armorType.value]);
    for (let prop of WFRP_Utility._prepareQualitiesFlaws(this.data).map(i => i = "<a class ='item-property'>" + i + "</a>"))
      properties.push(prop);
  ...
  _ammunitionExpandData()
  {
    const data = duplicate(this.data.data);
    let properties = [];
    properties.push(WFRP2E.ammunitionGroups[data.ammunitionType.value])
  ...
  postItem()
  {
    ...
    renderTemplate('systems/wfrp2e/templates/chat/post-item.html', chatData).then(html =>
    {
  ...
  _trappingChatData()
  {
    const data = duplicate(this.data.data);
    let properties = [
    `<b>${game.i18n.localize("ITEM.TrappingType")}</b>: ${WFRP2E.trappingCategories[data.trappingType.value]}`,
    ...
    `<b>${game.i18n.localize("Availability")}</b>: ${WFRP2E.availability[data.availability.value]}`
    ]
    return properties;
  }
  ...
  {
    let properties = [
      `<b>${game.i18n.localize("ITEM.MutationType")}</b>: ${WFRP2E.mutationTypes[this.data.data.mutationType.value]}`,
    ];
    if (this.data.data.modifier.value)
  ...
  _talentChatData()
  {
    const data = duplicate(this.data.data);
    let properties = [];
    properties.push(`<b>${game.i18n.localize("Max")}: </b> ${WFRP2E.talentMax[data.max.value]}`);
    if (data.tests.value)
  ...
  _careerChatData()
  {
    ...
    properties.push(`<b>${game.i18n.localize("Status")}</b>: ${WFRP2E.statusTiers[this.data.data.status.tier] + " " + this.data.data.status.standing}`);
    properties.push(`<b>${game.i18n.localize("Characteristics")}</b>: ${this.data.data.characteristics.map(i => i = " " + WFRP2E.characteristicsAbbrev[i])}`);
  ...
  _spellChatData()
  {
    const data = duplicate(this.data.data);
    let properties = [];
    if (WFRP2E.magicLores[data.lore.value])
      properties.push(`<b>${game.i18n.localize("Lore")}</b>: ${WFRP2E.magicLores[data.lore.value]}`);
    else
  ...
  _weaponChatData()
  {
    ...
    let properties = [
     ...
     if (data.weaponGroup.value)
      properties.push(`<b>Group</b>: ${WFRP2E.weaponGroups[data.weaponGroup.value]}`);
    if (data.range.value)
      properties.push(`<b>${game.i18n.localize("Range")}</b>: ${data.range.value}`);
    if (data.damage.value)
      properties.push(`<b>${game.i18n.localize("Damage")}</b>: ${data.damage.value}`);
    ...
    if (data.reach.value)
      properties.push(`<b>${game.i18n.localize("Reach")}</b>: ${WFRP2E.weaponReaches[data.reach.value] + " - " + WFRP2E.reachDescription[data.reach.value]}`);
    if (data.weaponDamage)
      properties.push(`<b>${game.i18n.localize("ITEM.WeaponDamaged")} ${data.weaponDamage} points</b>`)
    if (data.APdamage)
      properties.push(`${game.i18n.localize("ITEM.ShieldDamaged")} ${data.APdamage} points`)
  ...
  _armourChatData()
  {
    ...
    if (data.armorType.value)
      properties.push(`<b>${game.i18n.localize("ITEM.ArmourType")}</b>: ${WFRP2E.armorTypes[data.armorType.value]}`);
    if (data.penalty.value)
      properties.push(`<b>${game.i18n.localize("Penalty")}</b>: ${data.penalty.value}`);
    ...
    for (let loc in WFRP2E.locations)
      if (data.maxAP[loc])
        properties.push(`<b>${WFRP2E.locations[loc]} AP</b>: ${data.currentAP[loc]}/${data.maxAP[loc]}`);
  ...
  _ammunitionChatData()
  {
    const data = duplicate(this.data.data);
    let properties = [];
    properties.push(`<b>${game.i18n.localize("ITEM.AmmunitionType")}:</b> ${WFRP2E.ammunitionGroups[data.ammunitionType.value]}`)
...
// Assign ItemWfrp2e class to CONFIG
CONFIG.Item.entityClass = ItemWfrp2e;
