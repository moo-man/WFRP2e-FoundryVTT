class WFRP_Audio 
{
    static PlayContextAudio(context)
    {
        this.MatchContextAudio(context).then(sound => {
            console.log(`wfrp2e | Playing Sound: ${sound.file}`)
    ...
    {
      if (!game.settings.get("wfrp2e", "soundEffects") || !context)
        return {}
        
      try {
        let files = ""
        let file, group;
        await FilePicker.browse("user", `systems/wfrp2e/sounds`).then(resp => {
          files = resp.files
        })
      ...
      {
        console.log("wfrp2e | Sound Context Error: " + e)
      }
    }
}