...
Hooks.on("updateCombat", (combat) => {
    if (game.user.isGM && combat.data.round != 0 && combat.turns && combat.data.active)
    {
      ...
      if (game.settings.get("wfrp2e", "displayRoundSummary") && combat.current.turn == 0 && combat.current.round != 1)
        WFRP_Utility.displayRoundSummary(combat)

      if (game.settings.get("wfrp2e", "statusOnTurnStart"))
        WFRP_Utility.displayStatus(turn.token._id, combat.data.round);
  
      if (game.settings.get("wfrp2e", "focusOnTurnStart"))
      {
