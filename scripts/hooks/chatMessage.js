...
Hooks.on("chatMessage", (html, content, msg) => {
     ...
     * @returns {{amount: string, option: WFRP2E.creditOptions}}
     */
    function extractAmountAndOptionFromCommandLine(commands) {
        ...
        if (isAnAmount) {
            ...
            optionInCommandLine = WFRP2E.creditOptions.SPLIT;
        } else {
     ...
     * @returns {WFRP2E.creditOptions} an option
     */
    function getOption(optionInCommandLine) {
      return (typeof optionInCommandLine == "undefined") ? WFRP2E.creditOptions.SPLIT : optionInCommandLine;
    }    
...
// Roll on a table
    ...
    // Lookup a condition
        ...
        let closest = WFRP_Utility.matchClosest(WFRP2E.conditions, conditionInput);
        if (!WFRP2E.conditionDescriptions)
        {
            ui.notifications.error("No content found")
            return false
        }
        let description = WFRP2E.conditionDescriptions[closest];
        let name = WFRP2E.conditions[closest];
    ...
    // Availability test
        ...
        MarketWfrp2e.testForAvailability({settlement, rarity, modifier});
        return false;
    }
    // Pay commands
    else if (command === "/pay") {
        ...
        if (!game.user.isGM) {
            ...
            money = MarketWfrp2e.payCommand(amount, money);
            if (money)
                actor.updateEmbeddedEntity("OwnedItem", money);
        } else //If hes a gm, it generate a "Pay" card
            MarketWfrp2e.generatePayCard(amount, player);
        return false;
    }
    // Credit commands
        ...
        if (game.user.isGM) {
            MarketWfrp2e.generateCreditCard(amount, option);
        } else {
    ...
    //Help commands
        ...
        renderTemplate("systems/wfrp2e/templates/chat/chat-help-command.html", {
