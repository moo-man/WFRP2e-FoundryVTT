...
Hooks.on("createOwnedItem", (actor, item) => {
  ...
  catch (error)
  {
    ...
    if (item.type == "talent")
    {
      let charToIncrease = WFRP2E.talentBonuses[item.name.toLowerCase().trim()] // TODO: investigate why trim is needed here
      if (charToIncrease)
      {
    ...
    if (item.type == "trait")
    {
      if (actor.data.data.excludedTraits && actor.data.data.excludedTraits.length && actor.data.data.excludedTraits.includes(item._id))
        return
      let bonuses = WFRP2E.traitBonuses[item.name.toLowerCase().trim()] // TODO: investigate why trim is needed here
...
Hooks.on("deleteOwnedItem", (actor, item) => {
  if (item.type == "talent")
  {
    let charToDecrease = WFRP2E.talentBonuses[item.name.toLowerCase().trim()] // TODO: investigate why trim is needed here
