...
class WFRP_Utility
{
  ...
  static _spellDescription(spell)
  {
    ...
    else if (WFRP2E.loreEffect && WFRP2E.loreEffect[spell.data.lore.value])
      description += "\n\n <b>Lore:</b> " + WFRP2E.loreEffect[spell.data.lore.value];
    return description;
  }
   ...
   * 
   * @param {Object} AP     AP object defined in actor preparation (see ActorWfrp2e.prepareItems()) - consists of layers
    ...
    let qualities = item.data.qualities.value.split(",").map(function (item)
    {
      if (item)
      {
        ...
        if (!(Object.values(WFRP_Utility.qualityList()).includes(item) || (Object.values(WFRP_Utility.flawList()).includes(item)))) //if the quality does not show up in either quality or flaw list, add it
          WFRP2E.itemQualities[item.toLowerCase().trim()] = item;
        return item
      }
    });
    let flaws = item.data.flaws.value.split(",").map(function (item)
    {
      if (item)
      {
        ...
        if (!(Object.values(WFRP_Utility.flawList()).includes(item) || (Object.values(WFRP_Utility.qualityList()).includes(item)))) //if the quality does not show up in either quality or flaw list, add it
          WFRP2E.itemFlaws[item.toLowerCase().trim()] = item;
        return item;
      }
    });
  ...
  {
    let characteristics = {};
    let characteristicFormulae = WFRP2E.speciesCharacteristics[species];
    try
    {
      if (!characteristicFormulae) // If input species was not a valid key, try finding it as a value
        characteristicFormulae = WFRP2E.speciesCharacteristics[this.findKey(species, WFRP2E.species)]
    }
    catch (error)
    {
      ui.notifications.info("Could not find species " + species)
      console.log("wfrp2e | Could not find species " + species + ": " + error);
      throw error
    }

    for (let char in WFRP2E.characteristics)
    {
      if (average)
      {
  ...
  {
    let move = WFRP2E.speciesMovement[species];
    if (!move) // If input species was not a valid key, try finding it as a value
      move = WFRP2E.speciesMovement[this.findKey(species, WFRP2E.species)]
    return move;
  }
   ...
   * @param {String} location   Compendium to look into, format: <package.name> - "wfrp2e.trappings"
   */
  ...
  // Used to sort arrays based on string value (used in organizing skills to be alphabetical - see ActorWfrp2e.prepareItems())
  ...
  static qualityList()
  {
    let weapon = duplicate(WFRP2E.weaponQualities);
    let armor = duplicate(WFRP2E.armorQualities);
    let item = duplicate(WFRP2E.itemQualities);
  ...
  static flawList()
  {
    let weapon = duplicate(WFRP2E.weaponFlaws);
    let armor = duplicate(WFRP2E.armorFlaws);
    let item = duplicate(WFRP2E.itemFlaws);
    let list = mergeObject(weapon, mergeObject(item, armor))
    return list;
  }
  ...
  static _calculateAdvCost(currentAdvances, type)
  {
    ...
    if (index >= WFRP2E.xpCost[type].length)
      return WFRP2E.xpCost[WFRP2E.xpCost.length - 1];
    return WFRP2E.xpCost[type][index];
  }
  ...
  static displayStatus(tokenId, round = undefined)
  {
    ...
    chatOptions["template"] = "systems/wfrp2e/templates/chat/combat-status.html"
  ...
  static displayRoundSummary(combat)
  {
    ...
    if (["gmroll", "blindroll"].includes(chatOptions.rollMode)) chatOptions["whisper"] = ChatMessage.getWhisperIDs("GM");
    if (chatOptions.rollMode === "blindroll") chatOptions["blind"] = true;
    chatOptions["template"] = "systems/wfrp2e/templates/chat/round-summary.html"
  ...
  static parseConditions(effectList)
  {
    ...
    {
      let displayValue = (WFRP2E.conditions[c])
  ...
  {
    let symkey = WFRP_Utility.findKey(symptom.split("(")[0].trim(), WFRP2E.symptoms)
    let content = `<b>${symptom}</b>: ${WFRP2E.symptomDescriptions[symkey]}`;
    let chatOptions = {
      user: game.user._id,
      rollMode: game.settings.get("core", "rollMode"),
      content: content
    ...
    if (game.user.isGM)
    {
      content = `<b>${symptom} Treatment</b>: ${WFRP2E.symptomTreatment[symkey]}`;
      chatOptions = {
        user: game.user._id,
        rollMode: game.settings.get("core", "rollMode"),
        content: content
  ...
  static postProperty(property)
  {
    let properties = mergeObject(WFRP_Utility.qualityList(), WFRP_Utility.flawList()),
      propertyDescr = Object.assign(duplicate(WFRP2E.qualityDescriptions), WFRP2E.flawDescriptions),
      propertyKey;
  ...
  static handleTableClick(event)
  {
    ...
    {
      renderTemplate('systems/wfrp2e/templates/chat/table-dialog.html').then(html =>
      {
        new Dialog(
        {
  ...
  static handleConditionClick(event)
  {
    ...
    cond = cond.split(" ")[0]
    let condkey = WFRP_Utility.findKey(cond, WFRP2E.conditions);
    let condDescr = WFRP2E.conditionDescriptions[condkey];
  ...
  static handlePayClick(event)
  {
    ...
    if (game.user.isGM)
      MarketWfrp2e.generatePayCard(payString);
  }
  ...
  static handleCreditClick(event)
  {
    ...
    if (game.user.isGM)
      MarketWfrp2e.generateCreditCard(amt, option);
  ...
  static evalWeaponLength(weaponLength)
  {
    ...
    switch(weaponLength)
    {
      case game.i18n.localize('WFRP2E.Reach.Personal'):
        reach = 1;
        break;
      case game.i18n.localize('WFRP2E.Reach.VShort'):
        reach = 2;
        break;
      case game.i18n.localize('WFRP2E.Reach.Short'):
        reach = 3;
        break;
      case game.i18n.localize('WFRP2E.Reach.Average'):
        reach = 4;
        break;
      case game.i18n.localize('WFRP2E.Reach.Long'):
        reach = 5;
        break;
      case game.i18n.localize('WFRP2E.Reach.VLong'):
        reach = 6;
        break;
      case game.i18n.localize('WFRP2E.Reach.Massive'):
        reach = 7;
        break;
      default:
        break;
  ...
  {
    let morrsliebActive = canvas.scene.getFlag("wfrp2e", "morrslieb")
    morrsliebActive = !morrsliebActive
    await canvas.scene.setFlag("wfrp2e", "morrslieb", morrsliebActive)
