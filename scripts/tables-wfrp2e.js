...
class WFRP_Tables
{
  ...
  static formatChatRoll(table, options = {}, column = null)
  {
    ...
    switch (table)
    {
      ...
      case "career":
        return `<b>${this[table].name} - ${WFRP2E.species[column]}</b><br> <a class = "item-lookup">${result.name}</a> <br> <b>${game.i18n.localize("Roll")}:</b> ${result.roll}`;
      case "eyes":
      case "hair":
        return `<b>${this[table].name} - ${WFRP2E.species[column]}</b><br>${result.name}<br><b>${game.i18n.localize("Roll")}:</b> ${eval(result.roll)}`
