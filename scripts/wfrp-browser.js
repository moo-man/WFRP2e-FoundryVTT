/** Class for the WFRP2e Item Browser that collects all items in the world and compendia and
...
*/
class BrowserWfrp2e extends Application
{
  constructor(app)
  {
    super(app)
  ...
  static get defaultOptions()
  {
    ...
    options.id = "wfrp2e-browser";
    options.template = "systems/wfrp2e/templates/browser/browser.html"
    options.classes.push("wfrp2e", "wfrp-browser");
  ...
  getData() {
    ...
    data.talentMax = WFRP2E.talentMax;
    data.trappingTypes = WFRP2E.trappingTypes;
    data.lores = this.lores;
    data.characteristics = WFRP2E.characteristicsAbbrev;
  ...
  addItems(itemList)
  {
    ...
    this.lores = this.lores.map (p => {
      if (WFRP2E.magicLores[p])
        return WFRP2E.magicLores[p];
      else
        return p;
    })
    ...
    for (let filter in this.filters.dynamic)
    {
      if (this.filters.dynamic[filter].show && this.filters.dynamic[filter].value)
      {
        switch(filter)
        {
          ...
          case "melee":
            case "ranged":
            filteredItems = filteredItems.filter(i => i.type != "weapon" || filter == WFRP2E.groupToType[i.data.data.weaponGroup.value])
            break;
          case "weaponRange":
...
Hooks.on("renderCompendiumDirectory", (app, html, data) => {
  if (game.user.isGM || game.settings.get("wfrp2e", "playerBrowser"))
  {
...
Hooks.on('init', () => {
  if (!game.wfrpbrowser)
    game.wfrpbrowser = new BrowserWfrp2e();
})