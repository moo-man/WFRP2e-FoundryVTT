...
class GeneratorWfrp2e 
{
  ...
  static speciesStage()
  {
    ...
    renderTemplate("systems/wfrp2e/templates/chat/chargen/species-select.html", {species : WFRP2E.species}).then(html => {
  ...
  static rollSpecies(messageId, chosenSpecies = null)
  {
    ...
    {
      exp = 0;
      roll = {roll: game.i18n.localize("Choose"), value : chosenSpecies, name : WFRP2E.species[chosenSpecies], exp : 0}
    }
    else
    {
      exp = 20;
    ...
    let updateCardData = {roll : roll, species : WFRP2E.species}

    // Update the species selection menu to show what was rolled/chosen
    renderTemplate("systems/wfrp2e/templates/chat/chargen/species-select.html", updateCardData).then(html =>{
  ...
  static rollAttributes(species, exp = 0, reroll = false)
  {
    ...
    let dataTransfer = {
      ...
      type : "attributes",
      payload : {
        species: WFRP2E.species[species],
        characteristics : characteristics,
        movement : WFRP2E.speciesMovement[species],
        fate : WFRP2E.speciesFate[species],
        resilience : WFRP2E.speciesRes[species],
        exp : calcExp
      }
    }
    ...
    for (let abrev in WFRP2E.characteristicsAbbrev)
    {
      cardData.characteristics[WFRP2E.characteristicsAbbrev[abrev]] = dataTransfer.payload.characteristics[abrev]
    }
    cardData.speciesKey = species;
    cardData.extra = WFRP2E.speciesExtra[species]
    cardData.move = WFRP2E.speciesMovement[species]

    renderTemplate("systems/wfrp2e/templates/chat/chargen/attributes.html", cardData).then(html => {
      let chatData = WFRP_Utility.chatDataSetup(html)
  ...
  static speciesSkillsTalents(species, exp)
  {
    let cardData = {
      speciesKey : species,
      species : WFRP2E.species[species],
      speciesSkills : WFRP2E.speciesSkills[species],
      exp : exp
    }
    ...
    WFRP2E.speciesTalents[species].forEach(talent => {
        if (isNaN(talent))
        {
    ...
    let randomTalents = WFRP2E.speciesTalents[species][WFRP2E.speciesTalents[species].length-1]
    ...
    renderTemplate("systems/wfrp2e/templates/chat/chargen/species-skills-talents.html", cardData).then(html =>{
      let chatData = WFRP_Utility.chatDataSetup(html)
      ChatMessage.create(chatData);
    })
  }
  ...
  static async displayCareer(careerName, species, exp, isReroll, isChosen)
  {
    ...
    let cardData = {
      ...
      chosen : isChosen,
      speciesKey: species,
      trappings : WFRP2E.classTrappings[WFRP_Utility.matchClosest(WFRP2E.classTrappings, careerFound.data.data.class.value)] // Match closest is needed here (Academics/Academic)
    }

    // Show card with instructions and button
    renderTemplate("systems/wfrp2e/templates/chat/chargen/career-select.html", cardData).then(html => {
      let chatData = WFRP_Utility.chatDataSetup(html)
      ChatMessage.create(chatData);
    })
  }
  ...
  static async rollDetails(species)
  {
    ...
    age = new Roll(WFRP2E.speciesAge[species]).roll().total;
    heightRoll = new Roll(WFRP2E.speciesHeight[species].die).roll().total;
    hFeet = WFRP2E.speciesHeight[species].feet;
    hInches = WFRP2E.speciesHeight[species].inches + heightRoll;
    ...
    renderTemplate(`systems/wfrp2e/templates/chat/chargen/details.html`, cardData).then(html => {
      let chatData = WFRP_Utility.chatDataSetup(html)
      chatData["flags.transfer"] = JSON.stringify(dataTransfer);
      ChatMessage.create(chatData);
    })
