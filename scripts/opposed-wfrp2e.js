 ...
 * 
 * Targeted flow:
 * Every roll (see roll overrides, ActorWfrp2e) checks to see if a target is selected, if so, handleOpposed is called. See this function for details
...
class OpposedWFRP
{
  ...
  */
  static async checkPostModifiers(attacker, defender){
    ...
    if (game.settings.get("wfrp2e", "weaponLength") && attacker.testResult.postFunction == "weaponOverride" && defender.testResult.postFunction == "weaponOverride" && attacker.testResult.weapon.attackType == "melee" && defender.testResult.weapon.attackType == "melee"){
      WFRP_Utility.findKey(attacker.testResult.weapon.data.reach.value, WFRP2E.weaponReaches)
      let attackerReach = WFRP2E.reachNum[WFRP_Utility.findKey(attacker.testResult.weapon.data.reach.value, WFRP2E.weaponReaches)];
      let defenderReach = WFRP2E.reachNum[WFRP_Utility.findKey(defender.testResult.weapon.data.reach.value, WFRP2E.weaponReaches)];
    ...
    //Size Differences
    let sizeDiff = WFRP2E.actorSizeNums[attacker.testResult.size] - WFRP2E.actorSizeNums[defender.testResult.size]
  ...
  {
    Hooks.call("wfrp2e:preOpposedTestResult", attacker, defender)
    try
    {
      let opposeResult = {};
      ...
      Hooks.call("wfrp2e:opposedTestResult", opposeResult, attacker, defender)
      ...
      if (options.target)
      {
        ...
        renderTemplate("systems/wfrp2e/templates/chat/opposed-result.html", opposeResult).then(html =>
        {
          let chatOptions = {
            user: game.user._id,
            content: html,
  ...
  static calculateOpposedDamage(opposeData)
  {
      ...
      let sizeDiff = WFRP2E.actorSizeNums[opposeData.attackerTestResult.size] - WFRP2E.actorSizeNums[opposeData.defenderTestResult.size]
